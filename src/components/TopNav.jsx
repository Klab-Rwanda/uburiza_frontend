    ...
    <header className="bg-white border-b border-emerald-200 h-16 flex items-center justify-between px-8 sticky top-0 z-10 transition-colors duration-300">
      <div className="flex items-center space-x-12">
        ...
        <nav className="hidden md:flex space-x-6 text-sm font-medium">
          <button onClick={() => setView('Dashboard')} className={`${view === 'Dashboard' ? 'text-emerald-700' : 'text-emerald-500 hover:text-emerald-800'}`}>Courses</button>
          <button onClick={() => setView('Resources')} className={`${view === 'Resources' ? 'text-emerald-700' : 'text-emerald-500 hover:text-emerald-800'}`}>Resources</button>
          <button onClick={() => setView('Dashboard')} className={`${view === 'Dashboard' ? 'text-emerald-700' : 'text-emerald-500 hover:text-emerald-800'}`}>My Dashboard</button>
        </nav>
      </div>
      ...
      <div className="flex items-center space-x-6">
        <div className="relative">
          ...
          <input 
            ...
            className="pl-9 pr-4 py-2 border border-emerald-200 bg-white text-emerald-900 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 w-64 transition-colors duration-300"
          />
        </div>
        ...
        <button className="text-emerald-500 hover:text-emerald-800 relative">
